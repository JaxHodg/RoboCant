...
try:
    with conn.cursor() as cur:
        for headFile in headFiles:
            cur.execute(
                createTableQueryFromCSV(headFold + '\\' + headFile, headFile.split('.')[0])
            )
            conn.commit()
...
# with conn.cursor() as cur:
#     cur.execute("DELETE FROM camera WHERE num='offset';")

#     conn.commit()

try:
    with conn.cursor() as cur:
        cur.execute("INSERT INTO camera (num, val) VALUES ('offset', '0');")

        conn.commit()
except:
    print("DB already set up-2")